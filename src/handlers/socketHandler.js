...
// Login y Registro (admin o usuario)
const { addCredentials, checkCredentials } = require('./credentialHandler');	
const { addToStat, seeStats, addFriend, deleteFriend, sendMessage}  = require('./socialHandler');
	...
	io.on('connection', (socket) => {
		...
		socket.on('login', async ({ username, password, type }) => {
			...
			if (ok === 1) {
				...
				// …si quieres lógica extra para Admin:
				if (type === 'Admin') {
					socket.join(ADMIN_ROOM);
					// emitir initial_screen_states…
				}
			} else {
